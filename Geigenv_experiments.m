...
mu = 0.4; omega = mu*len; % operating frequency

% Settings for modulation
xi = 0.21; Omega = xi*len; % modulation frequency
...
r = 0.5; c = -0.5; % circle radius and center
theta = linspace(0, 2*pi, 500);
z = c + r*exp(1i*theta);  % Complex points on the circle
plot(real(z),imag(z),'--k','HandleVisibility','off');

% Iterate over k_tr
for k_tr = 20:-1:1

    % Compute the eigenvalues of the matrix \mathcal{G}
    O = diag(omega.*ones(1,2*k_tr+1)+[-k_tr:k_tr].*Omega);
    G = get_Gmat(k_tr,omega,Omega,rs,ks,vr,gamma,len,N);
    [V,D] = eig(G); eigenvals = diag(D);

    plot(real(eigenvals),imag(eigenvals),'.','Color',cmap(k_tr, :),'MarkerSize',20,'DisplayName',['$K = $ ',num2str(k_tr)])

end

legend('Interpreter','latex','Location','eastoutside')
xlabel('Re$(\varphi_i)$','Interpreter','latex')
ylabel('Im$(\varphi_i)$','Interpreter','latex')
axis equal
grid on
...
O = diag(omega.*ones(1,2*k_tr+1)+[-k_tr:k_tr].*Omega);
eks = linspace(0.5,0.99,10); % modulation amplitude of kappa
...
for epsilon_kappa = eks
    ...
    % Compute the total energy with the eigenvectors of G as the incident field
    G = get_Gmat(k_tr,omega,Omega,rs,ks,vr,gamma,len,N);
    [V,D] = eig(G); eigenvals = diag(D);
    [sorted_z, index] = sort(abs(eigenvals));
    eigenvals = eigenvals(index); V = V(:,index);
    for iv = 1:2*k_tr+1
        Vi_normalised = V(:,iv)./(norm(O*V(:,iv)));
...
for epsilon_rho = ers
    ...
    for iv = 1:2*k_tr+1
        Vi_normalised = V(:,iv)./(norm(O*V(:,iv)));
        Es(ir,iv) = get_E(G,Vi_normalised,O);
    end
...
%     omega = max(w_out);
    ...
    for iv = 1:2*k_tr+1
        Vi_normalised = V(:,iv)./(norm(O*V(:,iv)));
        Es(iN,iv) = get_E(G,Vi_normalised,O);
    end
...
for mu = mus
    ...
    for iv = 1:2*k_tr+1
        Vi_normalised = V(:,iv)./(norm(O*V(:,iv)));
        Es(im,iv) = get_E(G,Vi_normalised,O);
    end
